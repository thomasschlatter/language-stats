...
export class SkyOffice extends Room<OfficeState> {
  ...
  async onCreate(options: IRoomData) {
    ...
    this.onMessage(
      ...
      (client, message: { content: string }) => {
        ...
        if (message.content == "@foxy" || message.content == "@Foxy") {
          this.botMessage(client, message.content);
        }
        // if the message starts with @foxy, create bot message
        if (
          message.content.startsWith("@foxy") ||
          message.content.startsWith("@Foxy")
        ) {
          this.botMessage(client, message.content);
        }
    ...
    // when a player is near a bot, do something
    //this.onMessage(Message.NEAR_BOT, (client, content) => {
    //  // update the message array (so that players join later can also see the message)
    //  this.dispatcher.dispatch(new ChatBotMessageUpdateCommand(), {
    //    client,
    //    content
  ...
  onJoin(client: Client, options: any) {
    console.log(this.presence.subscriptions);
