...
export default class Network {
  ...
  async joinWorld(roomType: string, map: string, language?: string) {
    if (this.room) return; // already in a world — never open a second connection
    this.worldMap = map;
  ...
  initialize() {
    if (!this.room) return;
    this.lobby.leave();
    //console.log("You've joined the room:", this.room.id);
    ...
    this.room.state.players.onRemove = (player: IPlayer, key: string) => {
      ...
      store.dispatch(
        pushPlayerLeftMessage(player.name ? player.name : "Someone")
      );
    ...
    // when a bot sends a message
    this.room.onMessage(Message.ADD_BOT_CHAT_MESSAGE, ({ content }) => {
      phaserEvents.emit(Event.UPDATE_BOT_DIALOG_BUBBLE, content);
    });
  ...
  // method to register event listener and call back function when a message added
  onChatMessageAdded(
  ...
  // method to register event listener and call back function when a message added
  onBotChatMessageAdded(callback: (content: string) => void, context?: any) {
    phaserEvents.on(Event.UPDATE_BOT_DIALOG_BUBBLE, callback, context);
  }
